...
sub writeClassDoc
{
	...
	# Write out the x_*.cpp file
	my $file = "$outputdir/x_$fileName.cpp";
	open( XCPPFILE, ">$file" ) || die "Couldn't create $file\n";
	print STDERR "Writing x_$fileName.cpp\n" if ($debug);

	print XCPPFILE "//Auto-generated by $0. DO NOT EDIT.\n";
 	print XCPPFILE "#include <smoke.h>\n";
 	print XCPPFILE "#include <${libname}_smoke.h>\n";

        my ($methodCode, $switchCode, $addInclude) = generateAllMethods( $node );

	foreach my $incl (keys %{$addInclude}) {
	    die if $incl eq '';
	    print XCPPFILE "#include <$incl>\n";
	}	
	print XCPPFILE "\n";
	print XCPPFILE "class x_$fileName "; # currently xClassName == x_$fileName
	print XCPPFILE ": public $className " if $node->{BindingDerives};
        print XCPPFILE "{\n";
        print XCPPFILE $methodCode;
        print XCPPFILE "};\n";
	if(keys %{$node->{enumerations}}) {
	    print XCPPFILE "void xenum_$fileName(Smoke::EnumOperation xop, Smoke::Index xtype, void *&xdata, long &xvalue) {\n";
	    print XCPPFILE "    x_$fileName\::xenum_operation(xop, xtype, xdata, xvalue);\n";
	    print XCPPFILE "}\n";
	}
	print XCPPFILE "void xcall_$fileName(Smoke::Index xi, void *obj, Smoke::Stack args) {\n";
	print XCPPFILE $switchCode;
	print XCPPFILE "}\n";

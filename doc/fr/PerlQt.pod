...
grâce au module de David Faure.
...
L<SmokeQt
1.0|"http://webcvs.kde.org/cgi-bin/cvsweb.cgi/kdebindings/smoke"> La
...
un sous-répertoire (4 minutes pour un ordinateur moyen).
...
Le lieu d'installation préféré de SMOKE et de PUIC est le système de
...
lors de l'occurrence d'un évènement, puis démarrez la boucle évènementielle.

C'est tout. Qt gérera les évènements et les dirigera vers les
...
=head3 Un Widget perso

Réécrivons le programme "Hello World!" avec une version perso
de PushButton:
...
En d'autres termes, PerlQt utilise le système de comptage de
...
branchée à tout entrée d'un composant (y compris lui-même),
...
=item * Écrire l'implantaton de slots dans le Designer
...
=head1 Appendice: Comment utiliser la Documentation Qt

Lorsque vous voulez utiliser depuis PerlQt une classe ou méthode décrite
dans la L<documentation|"http://doc.trolltech.com"> Qt (voyez aussi le programme 'assistant'
livré avec Qt), vous devez suivre des règles de translation simples
...
pourra être donnée en argument à une méthode attendant une Qt::StringList.
 ...
 Liste des marshalleurs (PerlQt-3.006)
 -----------------------------------------------------------------
 ...
 QCString, -&, -*               <=>       chaîne Perl (utf8 ou octets, suivant contenu)
 QStringList, -&, -*             =>       référence à une liste de chaînes Perl (utf8)
 QString, -&, -*                 =>       chaîne Perl string (utf8 ou iso-latin1 ou locale)
 int&, -*                       <=>       entier Perl (IV)
 bool&, -*                      <=>       booléen Perl

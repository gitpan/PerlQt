...
use Qt qw(%Align %Direction %Orientation);
...
sub new {
    ...
    my $tab = QTabDialog->new(undef, 'top-level dialog');
    $tab->setCaption('Ugly Tab Dialog');

    # set up page one of the tab dialog
    my $w = QWidget->new($tab, 'page one');

    # stuff the labels and lineedits into a grid layout
    my $g = QGridLayout->new($w, 2, 2, 5);

    # two multilineedits in column 1
    my $ed1 = QMultiLineEdit->new($w);
    $g->addWidget($ed1, 0, 1);
    $ed1->setText('');
    $ed1->setMinimumSize(QSize->new(100, 10));
    my $ed2 = QMultiLineEdit->new($w);
    $g->addWidget($ed2, 1, 1);
    $ed2->setText('');
    $ed2->setMinimumSize(QSize->new(100, 10));
    ...
    my $mood =
	QSlider->new($Orientation{Vertical}, $w, 'mood slider');
    $mood->setRange(0, 127);
    ...
    my $labels = QBoxLayout->new($Direction{Down});
    $b->addLayout($labels);
    $b->addLayout($labels);
    $l = QLabel->new('Optimistic', $w, 'optimistic');
    ...
    $l = QLabel->new('Pessimistic', $w, 'pessimistic');
    $l->setFixedSize($l->sizeHint());

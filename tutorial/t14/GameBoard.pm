...
use Qt qw($app %Key %Frame %Weight);
...
sub new {
    ...
    my $frame = QFrame->new($self, 'cannonFrame');
    $frame->setFrameStyle($Frame{WinPanel} | $Frame{Sunken});
    ...
    my $accel = QAccel->new($self);
    $accel->connectItem($accel->insertItem($Key{Space}), $self, 'fire()');
    $accel->connectItem($accel->insertItem($Key{Q}), $app, 'quit()');

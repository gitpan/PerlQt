...
struct QPointArray {
    ...
    QPointArray(int {shortarrayitems(2)}, const QCOORD *{shortarray});
    ~QPointArray();
    ...
    void putPoints(int, int {shortarrayitems(3)}, const QCOORD *{shortarray});
    QPointArray quadBezier() const;
    bool resize(uint);
    void setPoint(uint, const QPoint &);
    void setPoint(uint, int, int);
    void setPoints(int {shortarrayitems(2)}, const QCOORD *{shortarray});
    uint size() const;
